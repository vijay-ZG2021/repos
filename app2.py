...
app.layout = dbc.Container([
    ...
    dbc.Row([
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H5("📊 Avg YTM", className="card-title"),
                    html.H3(id='portfolio-ytm', className="text-danger")
                ])
            ])
        ], width=2),
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H5("🎯 Avg Maturity", className="card-title"),
                    html.H3(id='portfolio-maturity', className="text-secondary")
                ])
            ])
        ], width=2)
    ], className="mb-4"),
...
@app.callback(
    ...
    Output('portfolio-ytm', 'children'),
    Output('portfolio-maturity', 'children'),
...
def update_dashboard(json_data):
    ...
    # Calculate average YTM and maturity
    total_market_value = sum(bond.market_price * qty for bond, qty in metrics['bonds'])
    avg_ytm = sum(bond.ytm * (bond.market_price * qty) / total_market_value for bond, qty in metrics['bonds']) * 100
    avg_maturity = sum(bond.years_to_maturity * (bond.market_price * qty) / total_market_value for bond, qty in metrics['bonds'])
    
    portfolio_ytm = f"{avg_ytm:.2f}%"
    portfolio_maturity = f"{avg_maturity:.2f} yrs"
    ...
    return total_value, portfolio_dv01, portfolio_duration, portfolio_convexity, portfolio_ytm, portfolio_maturity, executive_summary, bond_table, fig, scenario_table
